...
import { ERIC_TOKEN_ABI, TOKEN_BANK_PERMIT2_ABI, PERMIT2_ABI } from '@/constants/abis';
...
export default function TokenBankPermit2() {
  ...
  const { data: tokenBalance, refetch: refetchTokenBalance } = useReadContract({
    address: CONTRACTS_PERMIT2.EricToken as AddressType,
    abi: ERIC_TOKEN_ABI,
  ...
  useEffect(() => {
    if (signature && permit2Signature && address) {
      ...
      const permitTransfer = {
        permitted: {
          token: CONTRACTS_PERMIT2.EricToken,
          amount: amount,
        },
        nonce: BigInt(nonce),
