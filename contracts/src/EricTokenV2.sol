...
import {EricToken} from "./EricToken.sol";
 ...
 */
contract EricTokenV2 is EricToken {
    /**
     * @dev 构造函数
     * @param initialSupply 初始供应量（不含精度）
     */
    constructor(uint256 initialSupply) EricToken(initialSupply) {}

    /**
     * @dev 带回调的转账函数，如果目标地址是合约，则调用其tokensReceived函数
     * @param to 接收地址
     * @param amount 转账数量
     * @return 是否成功
     */
    function transferWithCallback(address to, uint256 amount) public returns (bool) {
        ...
        if (to.code.length > 0) {
            ...
            try ITokenReceiver(to).tokensReceived(msg.sender, amount) returns (bytes4 selector) {
                require(selector == ITokenReceiver.tokensReceived.selector, "Invalid tokensReceived implementation");
            } catch {
